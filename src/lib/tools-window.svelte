...
<div class="w-full h-full flex flex-col relative">
	<div class="w-full flex justify-between text-lg items-center text-neutral-400 bg-primary-600">
		...
		<div class="flex justify-end pr-2">
			<button class="p-0.5 hover:bg-primary-500 rounded" on:click={minimize}>
				<Icon color="currentcolor" src={BiMinus} />
			</button>
			{#if isMaximized}
				<button class="p-0.5 hover:bg-primary-500 rounded" on:click={restore}>
					<Icon color="currentcolor" src={FiChevronDown} />
				</button>
			{:else}
				<button class="p-0.5 hover:bg-primary-500 rounded" on:click={maximize}>
					<Icon color="currentcolor" src={FiChevronUp} />
				</button>
			{/if}
			<button class="p-0.5 hover:bg-primary-500 rounded" on:click={close}>
